var o=`<h1>Command Line Interface Guidelines</h1>
...
Offroad Engineer at <a href="https://smallstep.com/">Smallstep</a>, first engineer at Zipcar, co-founder Trove. <br />
...
<h3>Simple parts that work together {#simple-parts-that-work-together}</h3>
...
<h3>Consistency across programs {#consistency-across-programs}</h3>
...
<h3>Saying (just) enough {#saying-just-enough}</h3>
...
<h3>Ease of discovery {#ease-of-discovery}</h3>
...
<h3>Conversation as the norm {#conversation-as-the-norm}</h3>
...
<h3>Robustness {#robustness-principle}</h3>
...
<li>Bash: <a href="https://argbash.dev">argbash</a></li>
...
<li>Julia: <a href="https://github.com/carlobaldassi/ArgParse.jl">ArgParse.jl</a>, <a href="https://github.com/comonicon/Comonicon.jl">Comonicon.jl</a></li>
<li>Kotlin: <a href="https://ajalt.github.io/clikt/">clikt</a></li>
<li>Node: <a href="https://oclif.io/">oclif</a></li>
<li>Deno: <a href="https://jsr.io/@std/cli/doc/parse-args/~/parseArgs">parseArgs</a></li>
...
<li>Rust: <a href="https://docs.rs/clap">clap</a></li>
...
<h3>Help {#help}</h3>
<p><strong>Display extensive help text when asked.</strong>
Display help when passed <code>-h</code> or <code>--help</code> flags.
This also applies to subcommands which might have their own help text.</p>
<p><strong>Display concise help text by default.</strong>
When <code>myapp</code> or <code>myapp subcommand</code> requires arguments to function,
and is run with no arguments,
display concise help text.</p>
<p>You can ignore this guideline
if your program is interactive by default (e.g. <code>npm init</code>).</p>
...
<p><strong>Show full help when <code>-h</code> and <code>--help</code> is passed.</strong>
All of these should show help:</p>
<pre><code>$ myapp
$ myapp --help
$ myapp -h
</code></pre>
<p>Ignore any other flags and arguments that are passed—you should be able to add <code>-h</code> to the end of anything and it should show help.
...
<p><strong>Provide a support path for feedback and issues.</strong>
...
<p><strong>Use formatting in your help text.</strong>
...
<p>Rather than suggesting the corrected syntax, you might be tempted to just run it for them, as if they’d typed it right in the first place.
...
<h3>Documentation {#documentation}</h3>
...
— <a href="http://web.archive.org/web/20220609080931/https://homepage.cs.uri.edu/~thenry/resources/unix_art/ch01s06.html">Doug McIlroy</a></p>
</blockquote>
...
<p><strong>Make it easy to see the current state of the system.</strong>
...
<p><strong>Suggest commands the user should run.</strong>
...
<p><strong>Use color with intention.</strong>
...
<li>The <code>NO_COLOR</code> environment variable is set and it is not empty (regardless of its value).</li>
...
<pre><code class="language-shell-session">$ yubikey-agent -setup
...
<p><strong>By default, don’t output information that’s only understandable by the creators of the software.</strong>
...
Use a pager only if <code>stdin</code> or <code>stdout</code> is an interactive terminal.</p>
...
<p><strong>Catch errors and rewrite them for humans.</strong>
...
<p><em>Further reading: <a href="https://developers.google.com/tech-writing/error-messages">Google: Writing Helpful Error Messages</a>, <a href="https://www.nngroup.com/articles/error-message-guidelines">Nielsen Norman Group: Error-Message Guidelines</a></em></p>
<h3>Arguments and flags {#arguments-and-flags}</h3>
...
<li><code>-n</code>, <code>--dry-run</code>: Dry run. 
Do not run the command, but describe the changes that would occur if the command were run. For example, <code>rsync</code>, <code>git add</code>.</li>
...
<p><strong>If a flag can accept an optional value, allow a special word like “none”.</strong>
...
<p>This can be very confusing for the user—especially given that one of the most common things users do when trying to get a command to work is to hit the up arrow to get the last invocation, stick another option on the end, and run it again.
...
When a command accepts a secret, e.g. via a <code>--password</code> flag,
the flag value will leak the secret into <code>ps</code> output and potentially shell history.
And, this sort of flag encourages the use of insecure environment variables for secrets.
(Environment variables are insecure because they can often be read by other users, their values end up in debug logs, etc.)</p>
<p>Consider accepting sensitive data only via files, e.g. with a <code>--password-file</code> flag, or via <code>stdin</code>.
A <code>--password-file</code> flag allows a secret to be passed in discreetly, in a wide variety of contexts.</p>
<p>(It’s possible to pass a file’s contents into a flag in Bash by using <code>--password $(&lt; password.txt)</code>.
This approach has the same security problems mentioned above.
It’s best avoided.)</p>
<h3>Interactivity {#interactivity}</h3>
...
<h3>Robustness {#robustness-guidelines}</h3>
...
<p>One thing to be aware of: hiding logs behind progress bars when things go <em>well</em> makes it much easier for the user to understand what’s going on, but if there is an error, make sure you print out the logs.
...
<h3>Future-proofing {#future-proofing}</h3>
...
<h3>Signals and control characters {#signals}</h3>
...
<p>Your program should expect to be started in a situation where clean-up has not been run.
(See <a href="https://lwn.net/Articles/191059/">Crash-only software: More than meets the eye</a>.)</p>
<h3>Configuration {#configuration}</h3>
...
<li>Project-level configuration (e.g. <code>.env</code>)</li>
<li>User-level configuration</li>
<li>System wide configuration</li>
</ul>
<h3>Environment variables {#environment-variables}</h3>
...
<ul>
<li><code>NO_COLOR</code>, to disable color (see <a href="#output">Output</a>) or <code>FORCE_COLOR</code> to enable it and ignore the detection logic</li>
...
<h3>Naming {#naming}</h3>
<blockquote>
<p>“Note the obsessive use of abbreviations and avoidance of capital letters; [Unix] is a system invented by people to whom repetitive stress disorder is what black lung is to miners.
Long names get worn down to three-letter nubbins, like stones smoothed by a river.”
— Neal Stephenson, <em><a href="https://web.stanford.edu/class/cs81n/command.txt">In the Beginning was the Command Line</a></em></p>
</blockquote>
...
If you expect people to type your command name all day, make it easy on their hands.</p>
<p>A real-world example: long before Docker Compose was <code>docker compose</code>, it was <a href="https://github.com/aanand/fig/blob/0eb7d308615bae1ad4be1ca5112ac7b6b6cbfbaf/setup.py#L26"><code>plum</code></a>.
This turned out to be such an awkward, one-handed hopscotch that it was immediately renamed to <a href="https://github.com/aanand/fig/commit/0cafdc9c6c19dab2ef2795979dc8b2f48f623379"><code>fig</code></a>, which – as well as being shorter – flows much more easily.</p>
