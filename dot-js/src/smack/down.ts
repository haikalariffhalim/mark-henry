...
const os = platform()
const usage =
	`\n${ui.magenta.bold`dotjs`} is a fast Markdown runtime and bundler. ` +
	`${ui.gray(`(${version})`)}\n
${ui.bold(`Usage: dotjs <commmand> ${ui.cyan`[...flags]`} [...args]`)}\n
${ui.bold`Commands:`}
${ui.table({ columns: [26, 40] })([
		[ui.magenta.bold`run` + ui.gray`? app.md`, 'Execute a file with dotjs,'],
		...
		[ui.cyan.bold`upgrade`, 'Upgrade to the latest version of dotjs'],
	])}
${ui.bold`Flags:`}
...
Learn more about dotjs:    ${ui.blue`https://hreff.dev/dotjs`}\n`
...
function parseArgs(args: string[]) {
	...
	if (!semver.satisfies(Bun.version, dependencies.bun)) {
		console.log(usage)
		console.error(
			`\ndotjs requires Bun version ${dependencies.bun}, but found ${Bun.version}.\n` +
	...
	async function sourceToMarkdown(source: Source) {
		if (source.type === 'directory') {
			for (const defaultFile of ['introduction.md', 'guidelinemd', 'index.md']) {
			...
			if (source.type === 'directory') {
				throw new Error('dotjs run requires a file as input.')
			}
		}
	...
	try {
		if (command === 'upgrade') {
			const bin = (await $`which dotjs`.text()).replace(/dotjs/g, '')
			const packageManager = bin.includes('bun') ? 'bun' : bin.includes('pnpm') ? 'pnpm' : 'npm'
			await $`${packageManager} i -g dotjs@latest`
			process.exit(0)
		}
		if (flags.help) {
			console.log(usage)
		...
		if (command === 'sync') {
			if (args.length !== 2) {
				throw new Error('dotjs sync requires two arguments: source and destination.')
			}
			const source = await resolveSource(args[0])
			if (source.type === 'directory') {
				throw new Error('dotjs sync requires a file as the source.')
			}
			const destination = await resolveSource(args[1])
			if (destination.type === 'file' || destination.type === 'url') {
				throw new Error('dotjs sync requires a directory as the destination.')
			}
			...
			console.log(ui.bold`dotjs sync`)
			if (flags.print) console.log()
		...
		if (args.length !== 1) {
			throw new Error('dotjs run requires a single argument: source (path, URL, etc.)')
		}
		...
		const dot.js = `${process.env.HOME}/.dotjs`
		const filename = `${dot.js}/tmp/${sha256(crypto.randomUUID())}.ts`
		...
		try {
			...
			await $`rm -rf ${dot.js}/tmp/`
			process.exit(shellOutput?.exitCode ?? 0)
		}
	} catch (dotjsError) {
		console.log('\n' + ui.box.top(`Use ${ui.bold('dotjs -h')} for help`, ui.red) + '\n')
		console.log(dotjsError)
