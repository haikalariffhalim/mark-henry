...
pub fn create_ico(
    ...
    for (png_data, _) in icon_entries {
        ...
        let entry =
            IconDirEntry::encode(&icon_img).map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
    ...
    println!(
        "Created {:?} ({} sizes: 16–256 px)",
        output_path, valid_count
    );
    Ok(())
}
...
pub fn create_social_media_png(
    ...
    let logo_size = 512;
    let mut logo_pixmap = Pixmap::new(logo_size, logo_size)
        .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "Failed to create logo pixmap"))?;
    ...
    let mut canvas = Pixmap::new(canvas_width, canvas_height)
        .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "Failed to create canvas pixmap"))?;
